...
public class Perlin {
    public static float[,] GeneratePerlinArray(
        int width, int height, int octaves, float scale, float persistence, float lacunarity, int seed) {
        var amplitude = 1f;
        var frequency = 1f;
        ...
        for (var i = 0; i < octaves; i++) {
            var xOffset = rng.Next(-100000, 100000);
            var yOffset = rng.Next(-100000, 100000);
    ...
    public static void AddOctave(float[,] data, int xOffset, int yOffset, float scale, float amplitude,
        float frequency) {
    ...
    private static float PerlinValue(int x, int y, float scale, int xOffset, int yOffset, float frequency) {
        var xSample = x / scale * frequency + xOffset;
        var ySample = y / scale * frequency + yOffset;
